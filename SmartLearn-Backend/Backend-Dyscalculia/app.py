...
MODEL_PATH = BASE_DIR / "models" / "digit_tracing_model.h5"


def get_allowed_origins() -> list[str]:
    """Read a comma-separated list of permitted frontend origins."""
...
# Load at import time so both `python app.py` and Gunicorn workers have a
# ready model before the first prediction request.
load_digit_model()
...
@app.route("/api/dyscalculia/tracing/predict", methods=["POST"])
def predict():
