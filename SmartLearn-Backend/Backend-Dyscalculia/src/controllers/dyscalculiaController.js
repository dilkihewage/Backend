...
import { dyscalculiaOverview } from '../config/overviewData.js';
import { DyscalculiaSession } from '../models/DyscalculiaSession.js';
...
const findGameByKey = (gameKey) => {
  for (const section of dyscalculiaOverview.sections) {
...
export const startSession = asyncHandler(async (req, res) => {
  ...
  const session = await DyscalculiaSession.create({
    userId,
...
export const listSessions = asyncHandler(async (req, res) => {
  ...
  const sessions = await DyscalculiaSession.find(filter)
    .sort({ startedAt: -1 })
    .limit(limit)
    .lean();
...
export const getSessionById = asyncHandler(async (req, res) => {
  ...
  const session = await DyscalculiaSession.findById(sessionId);
...
export const completeSession = asyncHandler(async (req, res) => {
  ...
  if (session.completedAt && session.status === 'completed') {
    const progress = await UserProgress.findOne({ userId, moduleId: 'dyscalculia' }).lean();
...
export const getProgress = asyncHandler(async (req, res) => {
  ...
  const progress = await UserProgress.findOne({ userId, moduleId: 'dyscalculia' }).lean();
  const recentSessions = await DyscalculiaSession.find({ userId, moduleId: 'dyscalculia' })
    .sort({ startedAt: -1 })
    .limit(10)
    .lean();
